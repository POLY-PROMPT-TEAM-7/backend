...
## OVERVIEW
LangGraph workflow nodes for knowledge extraction, validation, and graph construction.

## STRUCTURE
```
lib/backend_placeholder/nodes/
├── extract_graph.py      # GPT-4o LLM extraction
├── validate_graph.py     # Entity/relationship validation
├── mkgraph.py            # NetworkX graph construction
...
| Task | Location | Notes |
|------|----------|-------|
| Entity extraction | extract_graph.py | GPT-4o with structured output |
...
- **Graph construction**: Filter entities by type (concepts, theories, persons, methods)
...
5. **mkgraph** → Builds KnowledgeGraph from validated data
6. **END**
